...
class DeviceModel:
    ...
    def __init__(self, deviceName, BLEDevice, callback_method, imu_idx=0, callback_numeric=None):
        ...
        self.callback_numeric = callback_numeric

        self.deviceData = {}
        self.TempBytes = []

        # 数值缓存（不含字符串）
        self.q_xyzw = None  # [x,y,z,w]
        self.g_xyz = None  # [gx,gy,gz]
    ...
    def processData(self, Bytes):
        if Bytes[1] == 0x61:
            ...
            self.g_xyz = [float(Gx), float(Gy), float(Gz)]
            ...
            if Bytes[2] == 0x51:
                ...
                self.q_xyzw = [float(Q1), float(Q2), float(Q3), float(Q0)]
            else:
                pass

        if self.callback_numeric and (self.q_xyzw is not None) and (self.g_xyz is not None):
            self.callback_numeric(self.imu_idx, self.q_xyzw, self.g_xyz)
