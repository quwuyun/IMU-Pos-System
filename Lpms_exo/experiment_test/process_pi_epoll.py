...
class imu_posture:
    def __init__(self, ports, baudrate=115200):
        ...
        # Store solved qpos+qvel rows locally for CSV output.
        self.output = []
        ...
        while True:
            ...
            if not self.calibration_done:
                ...
                with self.output_lock:
                    self.output.append(np.concatenate((self.data.qpos.copy(), self.data.qvel.copy())))
                did_work = True
    ...
    def save_qpos_output(self):
        """Save solved qpos rows followed by qvel rows in one CSV file."""
        with self.output_lock:
            rows = [row.copy() for row in self.output]

        if not rows:
            print("No qpos/qvel data to save.")
            return None

        filename = './data' + str(datetime.datetime.now().strftime('%Y年%m月%d日%H时%M分%S秒%f')[:-3]) + '.csv'
        with open(filename, 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerows(rows)
        print(f"Saved {len(rows)} qpos/qvel rows to {filename}")
        return filename
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("\nKeyboardInterrupt received, saving qpos/qvel data...")
    finally:
        read_imu.save_qpos_output()
        read_imu.close()
